...
// Fallback to empty map: with query logging disabled the API returns [] not a map. See github.com/Panonim/dynacat/issues/289
...
// Fallback to empty map: with privacy enabled `top_ads` is returned as an empty array.
type pihole5TopBlockedDomains map[string]int

func (p *pihole5TopBlockedDomains) UnmarshalJSON(data []byte) error {
	// NOTE: don't use the pihole5TopBlockedDomains type here - infinite UnmarshalJSON recursion.
...
func fetchPiholeStats(
	...
	if includeGraph && seriesErr == nil {
		if len(seriesResponse.History) != 145 {
			...
			// v6 returns 145 points instead of 144; drop the oldest.
